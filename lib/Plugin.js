...
SpritesmithPlugin.prototype = {
        ...
        async.waterfall([
            ...
            function (spritesmithResult, callback) {
                async.parallel([
                    ...
                    this.writeImage.bind(this, this.options.target.image, spritesmithResult)
                ], callback);
            }.bind(this)
        ], compileCallback);
    },
    writeCSS: function (spritesmithResult, callback) {
        ...
        async.series([
            mkdirp.bind(null, path.dirname(this.options.target.css)),
    ...
    convertSpritesmithResultToSpritesheetTemplatesFormat: function (spritesmithResult) {
        var generateSpriteName = this.options.apiOptions.generateSpriteName;
        var sprites = _.map(
        ...
        var spritesheet = _.extend(
            {image: this.options.apiOptions.cssImageRef},
    ...
    writeImage: function (fileName, spritesmithResult, callback) {
        async.series([
